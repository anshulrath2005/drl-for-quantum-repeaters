...
class QuantumRepeaterParallelEnv(ParallelEnv):
    ...
    def __init__(self, n_segments=4, tau_c=10.0, p_gen=0.1, p_swap=0.5, normalized=True):
        ...
        self.observation_spaces = {agent: spaces.Box(low=-1, high=1e5, shape=(3, self.n + 1, self.n + 1), dtype=np.float32) for agent in self.possible_agents}
        ...
        self.messages = np.full((self.n - 1, 2), -1, dtype=np.float32)
    ...
    def _get_obs(self, agent_name):
        ...
        obs = np.full(shape=(3, self.n + 1, self.n + 1), fill_value=-1, dtype=np.float32)
        ...
        # 2. Check the "Inbox" on the Communication Plane (Layer 2)
        # What did the Left Neighbor (Node node_id - 1) send to its Right?
        if node_id > 1: 
            left_neighbor_agent_idx = (node_id - 1) - 1
            obs[2][node_id - 1][node_id] = self.messages[left_neighbor_agent_idx][1]
            
        # What did the Right Neighbor (Node node_id + 1) send to its Left?
        if node_id < self.n - 1:
            right_neighbor_agent_idx = (node_id + 1) - 1
            obs[2][node_id][node_id + 1] = self.messages[right_neighbor_agent_idx][0]
        
        # 3. Normalize only the physical ages (Leave the messages as hard 0, 1, or -1)
        if self.normalized:
    ...
    def step(self, actions):
        ...
        new_messages = np.full((self.n - 1, 2), -1, dtype=np.float32)
        ...
        for agent_name, action in actions.items():
            ...
            # Extract the 2 Communication Bits
            left_msg = action[idx]
            right_msg = action[idx + 1]
            
            new_messages[node_id - 1][0] = left_msg
            new_messages[node_id - 1][1] = right_msg
        ...
        # Update the master message board for the next turn
