...
def evaluate(model_path, p_swap_val, json_filename, episodes=30, num_cores=6):
    ...
    print(f"Mean successes: {np.mean(success_counts):.2f}")
    if all_link_ages:
        ...
        print(f"Saved comprehensive metrics to {json_filename}")

if __name__ == "__main__":
    print("\n========================================================")
    print("TEST 4: The Baseline Agent in an Imperfect Environment")
    print("Testing if an AI trained purely on 50% noise from step 0 performs worse than curriculum.")
    print("========================================================")
    evaluate("models/marl_comm_agent/noisy_baseline/quantum_repeater_marl_agent", 0.5, "data/comm_test4_ages.json")
